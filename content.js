function init() {
  ...
  bademsavarStorage.then(function (res) {
    bademsavarBlockedDomainsArr =
      res.bademsavarBlockedDomains !== undefined
        ? res.bademsavarBlockedDomains
        : [];
...
function hideResults(blockedDomains) {
  ...
  for (i = 0; i < results.length; i++) {
    for (j = 0; j < blockedDomains.length; j++) {
      if (
        results[i].firstChild &&
        typeof results[i].firstChild.hostname != "undefined" &&
        results[i].parentNode.parentNode.parentNode
      ) {
        if (
          results[i].firstChild.hostname.includes(blockedDomains[j]) == true
        ) {
...
function hideNewsList(blockedDomains) {
  ...
  for (j = 0; j < blockedDomains.length; j++) {
    if (
      newsList &&
      typeof newsList.textContent != "undefined" &&
      newsList.parentNode.parentNode.parentNode
    ) {
...
function hideInnerCards(blockedDomains) {
  ...
  for (let i = 0; i < innerCards.length; i++) {
    for (let j = 0; j < blockedDomains.length; j++) {
      if (
        typeof innerCards[i].innerHTML != "undefined" &&
        innerCards[i].parentElement &&
        innerCards[i].innerHTML.includes(blockedDomains[j]) == true
      ) {
...
function hideLinks(blockedDomains) {
  ...
  for (let i = 0; i < links.length; i++) {
    for (let j = 0; j < blockedDomains.length; j++) {
      if (
        typeof links[i].hostname != "undefined" &&
        links[i].parentElement &&
        links[i].hostname.includes(blockedDomains[j]) == true
      ) {
  ...
  for (let i = 0; i < links.length; i++) {
    for (let j = 0; j < blockedDomains.length; j++) {
      if (
        typeof links[i].innerHTML != "undefined" &&
        links[i].parentElement &&
        links[i].innerHTML.includes(blockedDomains[j]) == true
      ) {
        links[i].parentElement.style.display = "none";
        break;
      }
...
function hideNews(blockedDomains) {
  ...
  for (i = 0; i < news.length; i++) {
    for (j = 0; j < blockedDomains.length; j++) {
      if (
        news[i].firstChild.firstChild &&
        typeof news[i].firstChild.firstChild.hostname != "undefined"
      ) {
        if (
          news[i].firstChild.firstChild.hostname.includes(blockedDomains[j]) ==
          true
        ) {
